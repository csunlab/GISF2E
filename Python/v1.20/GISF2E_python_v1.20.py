...
def add_field(field_names,featureIndex):
    for name in range(len(field_names)):
            ...
            feature = layer.GetFeature(featureIndex) #lets get the first feature 
...
while True:
    option = raw_input('Do you want to use input/output folders (1) or type your files addresses (2)? Option 1 or 2: ')
    if option == '1':
        sf = shapefile.Reader(path+'/input/'+os.listdir(path+'/input/')[0])
        path_nodes = path+ '/output/Nodes_'+os.listdir(path+'/input/')[0][:-4]+'.shp'
        path_links = path+ '/output/Edges_'+os.listdir(path+'/input/')[0][:-4]+'.shp'
        path_edgelist = path+ '/output/Edgelist_'+os.listdir(path+'/input/')[0][:-4]+'.csv'
        break
    elif option == '2':
        sf = shapefile.Reader(raw_input("Please type where your file is located (Example:C:\Users\UIC\Documents\Shape_File.shp): "))
        path_nodes = raw_input("Please type where you want the nodes shapefile to be located(Example:C:\Users\UIC\Documents\Nodes_File.shp): ")
        path_links = raw_input("Please type where you want the links shapefile to be located(Example:C:\Users\UIC\Documents\Links_File.shp): ")
        path_edgelist = raw_input("Please type where you want the Edgelist to be located(Example:C:\Users\UIC\Documents\Edgelist.csv): ")
        break

osm = raw_input("Is your root file from Open Street Maps (Y/N)? ")
osm = osm.lower()
start = time.time()
...
for feature in sf.shapeRecords():
    ...
    n += 1
    if ((n-1) % 800) == 0: #The number 800 was chosen because it's shown the fastest results in a range of numbers from 100 to 1000
...
if utm_zone[-1] == "S":    
    p = Proj("+proj=utm +zone="+str(int(utm_zone[:-1]))+", +south +ellps=WGS84")
else:
    p = Proj("+proj=utm +zone="+str(int(utm_zone[:-1]))+", +ellps=WGS84")
...
else: # no test for non-OSM maps
    b = filter(lambda x: len(x[1]) >= 2, zip(a.keys(),a.values()))
for h in (b):
    for k in range(len(h[1])):
#        testando['continuidade'][h[1][k]] = 2
        testando.loc[[h[1][k]],'continuidade'] = 2
b = b + b1
points = sorted(list(testando.groupby('continuidade').groups[0]) + list(testando.groupby('continuidade').groups[2]))
testando.loc[points,'Node'] = range(1,len(points)+1)

print '\n'+str(time.time() - start)+'s - Reading and arranging are done!'
sys.stdout.flush()

#creating nodes shapefile
spatialReference = osgeo.osr.SpatialReference() #will create a spatial reference locally to tell the system what the reference will be stored
spatialReference.ImportFromProj4('+proj=utm +zone='+utm_zone+' ellps=WGS84 +datum=WGS84 +units=m') #here we define the utm zone as well as the world geodetic system(wgs)
driver = osgeo.ogr.GetDriverByName('ESRI Shapefile') # will select the driver for our shp-file creation.
shapeData = driver.CreateDataSource(path_nodes) #so there we will store our data
...
new_field = ogr.FieldDefn('Node', ogr.OFTInteger) #add a field 'Node' stored as integer
layer.CreateField(new_field) #self explaining
new_field = ogr.FieldDefn('coord_x', ogr.OFTReal) #add a field 'coord_x' stored as real
layer.CreateField(new_field) #self explaining
new_field = ogr.FieldDefn('coord_y', ogr.OFTReal) #add a field 'coord_y' stored as real
layer.CreateField(new_field) #self explaining
if osm == 'y'or osm == 'yes' or osm == 'y': #for osm maps
    new_field = ogr.FieldDefn('bridge', ogr.OFTInteger) #we will create a new field called bridge as integer
    layer.CreateField(new_field) #self explaining
    new_field = ogr.FieldDefn('tunnel', ogr.OFTInteger) #and another field called 'tunnel' stored as integer
    layer.CreateField(new_field) #self explaining
...
#Cut the raw links and Creat the edges shapefile
...
spatialReference.ImportFromProj4('+proj=utm +zone='+utm_zone+' ellps=WGS84 +datum=WGS84 +units=m') #here we define the utm zone as well as the world geodetic system(wgs)
driver = osgeo.ogr.GetDriverByName('ESRI Shapefile') # will select the driver for our shp-file creation.
shapeData = driver.CreateDataSource(path_links) #so there we will store our data
...
#several tests in order to create the edges shapefile correctly
...
edgelist.to_csv(path_edgelist,index=False)
